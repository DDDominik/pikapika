$(document).ready(function(){
	...
	$.post("handle/loadScroll.php",{cid:0},function(data){
		var list = eval("("+data+")") ;
	...
	$.post("handle/loadIndex.php",{cid:0},function(data){
		var list = eval("("+data+")") ;
		for(var i = 0 ; i < list.message.length ; i++)
		{
			var info = list.message[i] ;
	...
	for(var i = 1 ; i < 5 ; i++)
	{	
		$.post("handle/loadIndexByCategory.php",{cid:i},function(data){
			var list = eval("("+data.substring(0,data.lastIndexOf("}")+1)+")") ;
			var c = data.substring(data.lastIndexOf("}")+1) ;
			for(var j = 0 ; j < list.message.length ; j++)
			{
				var info = list.message[j] ;
				$(".category_"+c).append('<li id="v_'+info.video_id+'"><a href="html/play.html?id='+info.video_id+'"><div class="vt_list"><div><span class="vt_name" >'+info.name+'</span></div><img src="'+'info/video/'+info.video_id+'/'+info.picture+'"></div></a></li>') ;
				var top = $(".item_two").find("#v_"+info.video_id).offset().top - 150 ;
				var left = $(".item_two").find("#v_"+info.video_id).offset().left - 60 ;
